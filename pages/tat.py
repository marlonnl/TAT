...
from utils import get_dataframe, no_file, show_date_badge
...
protocolo_dtype = CategoricalDtype(categories=["SEPSE", "Janela AVC", "Dor Torácica"])
...
def _grupos_protocolo(protocolo: str, exame: str) -> list[dict]:
    """Monta a lista de grupos de exa,es para os protocolos (Sepse, AVC, DT)."""
...
def _grupos_manchester(protocolo: str) -> list[dict]:
    """Monta a lista de grupos de exames para as classificações Manchester."""
    return [
        {
            "exames": EXAMES[i],
...
def _criar_tabela_referencia(protocolo: str) -> None:
    ...
    st.markdown(
        f"##### :{COR[protocolo_]}[:material/radio_button_checked:] *{protocolo}*"
    )

    metas_fmt = [str(pd.Timedelta(minutes=m)).split()[-1] for m in METAS[protocolo_]]
...
def _bloco_metricas(edited_df: pd.DataFrame, meta: pd.Timedelta) -> None:
    ...
    c.metric(
        ...
        value=tmc_f,
        delta=tmc_dif,
        delta_arrow=tmc_arrow[0],
        delta_color=tmc_arrow[1],
        border=True,
    )
    d.metric(
        ...
        value=tml_f,
        delta=tml_dif,
        delta_arrow=tml_arrow[0],
        delta_color=tml_arrow[1],
        border=True,
    )
    e.metric(
        label="TAT",
        value=tat_f,
        delta=tat_dif,
        delta_arrow=tat_arrow[0],
        delta_color=tat_arrow[1],
        border=True,
    )
...
def mostrar_protocolo_tab(
    ...
    with tab:
        ...
        if titulo_header:
            st.markdown(
                f"##### :{COR[protocolo_]}[:material/radio_button_checked:] {titulo_header}"
            )
        else:
            # Protocolo simples: exibe o nome do exame que está em grupos[0]
            exame = grupos[0]["exames"][0]
            st.markdown(
                f"##### :{COR[protocolo_]}[:material/radio_button_checked:] Protocolo de *{protocolo}*: `{exame}`"
            )
        ...
        if is_manchester:
            ...
            opcoes = [
                str(g["meta"].round("1s")).split()[-1] for g in grupos  # ex: "0:20:00"
            ]
...
if data is not None:
    ...
    # data = st.session_state["dataset"]["df"]
    ...
    ## Protocolos
    mostrar_protocolo_tab(
        sepse_tab, sepse_df, "sepse_df", "Sepse", _grupos_protocolo("sepse", "LACTATO")
    )
    mostrar_protocolo_tab(
        card_tab,
        trp_df,
        "trp_df",
        "Dor torácica",
        _grupos_protocolo("dor torácica", "TRP"),
    )
    mostrar_protocolo_tab(
        avc_tab, avc_df, "avc_df", "Janela AVC", _grupos_protocolo("janela avc", "TAP")
    )
    ...
    mostrar_protocolo_tab(
        ...
        "murgente",
        "Muito Urgente",
        _grupos_manchester("muito urgente"),
        "Muito Urgente",
    )
    mostrar_protocolo_tab(
        urgente_tab,
    ...
    mostrar_protocolo_tab(
        ...
        "purgente",
        "Pouco Urgente",
        _grupos_manchester("pouco urgente"),
        "Pouco urgente",
    )
